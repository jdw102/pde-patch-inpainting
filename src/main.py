...
def test(damage_rect, image_name, name, patch_width=8.0, alpha=0.43, iterations=1.0, texture_radius=20, inpainting_radius=3, algorithm=cv.INPAINT_NS, synth_rect=None):
    ...
    restored_image = transfer_restore(damaged_image, mask, damage_rect, name, True, patch_width, alpha, iterations, texture_radius, inpainting_radius, algorithm)
    ...
    print(calculate_error(extract_rectangle(cv.imread(image_name), damage_rect), extract_rectangle(restored_image, damage_rect)))
