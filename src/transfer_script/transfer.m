a = im2double(imread('C:\Users\jwort\PycharmProjects\ece588\data\texture.jpg')); % Input Texture
b = im2double(imread('C:\Users\jwort\PycharmProjects\ece588\data\target.jpg')); % Input Image
...
for p = 1:iter
    for i = [1:floor(m1/w),(m-o)/w]
        for j = [1:floor(n1/w),(n-o)/w]
            ...
            x = (j - 1) * w;
            zz = j * w + o;
            t = mask_out((i-1)*w+1:i*w+o,(j-1)*w+1:j*w+o);
            if (all(all(t)))
    ...
    outputTexture = zeros(m,n);
    outputTexture1 = zeros(m,n,3);
    imwrite(output, append(append('../../data/texture_patch', int2str(p)), ".jpg"))
end
